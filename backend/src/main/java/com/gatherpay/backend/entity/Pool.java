...
public class Pool {
    ...
    @Column(length = 160)
    private String payoutTriggeredBy;

    @Column
    private Instant payoutTriggeredAt;
    ...
    @OneToMany(mappedBy = "pool", cascade = CascadeType.ALL, orphanRemoval = true, fetch = FetchType.EAGER)
    private List<PoolChatMessage> chatMessages = new ArrayList<>();
    ...
    public String getPayoutTriggeredBy() {
        return payoutTriggeredBy;
    }

    public void setPayoutTriggeredBy(String payoutTriggeredBy) {
        this.payoutTriggeredBy = payoutTriggeredBy;
    }

    public Instant getPayoutTriggeredAt() {
        return payoutTriggeredAt;
    }

    public void setPayoutTriggeredAt(Instant payoutTriggeredAt) {
        this.payoutTriggeredAt = payoutTriggeredAt;
    }

    public List<PoolChatMessage> getChatMessages() {
        return chatMessages;
    }

    public void setChatMessages(List<PoolChatMessage> chatMessages) {
        this.chatMessages.clear();
        for (PoolChatMessage chatMessage : chatMessages) {
            chatMessage.setPool(this);
