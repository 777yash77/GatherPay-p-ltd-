...
import com.gatherpay.backend.repository.PoolChatMessageRepository;
...
public class PoolService {
    ...
    private final PoolChatMessageRepository poolChatMessageRepository;
    private final UserAccountRepository userAccountRepository;
    private final NotificationService notificationService;

    public PoolService(
            PoolRepository poolRepository,
            PoolChatMessageRepository poolChatMessageRepository,
            UserAccountRepository userAccountRepository,
            NotificationService notificationService
    ) {
        this.poolRepository = poolRepository;
        this.poolChatMessageRepository = poolChatMessageRepository;
    ...
    public PoolResponse contribute(UUID poolId, ContributionRequest request, String userEmail) {
        ...
        if (pool.isPayoutCompleted()) {
            throw new BadRequestException("This pool has already been paid out");
        }
    ...
    @Transactional(readOnly = true)
    public List<PoolChatMessageResponse> getChatMessages(UUID poolId, String userEmail) {
        UserAccount currentUser = findUser(userEmail);
        Pool pool = findPool(poolId);
        ensurePoolMember(pool, currentUser);

        return poolChatMessageRepository.findByPoolIdOrderByCreatedAtAsc(poolId).stream()
                .map(this::toChatResponse)
                .toList();
    }

    public PoolChatMessageResponse addChatMessage(UUID poolId, PoolChatMessageRequest request, String userEmail) {
        UserAccount currentUser = findUser(userEmail);
        Pool pool = findPool(poolId);
        ensurePoolMember(pool, currentUser);

        PoolChatMessage message = new PoolChatMessage();
        message.setPool(pool);
        message.setSenderName(currentUser.getName());
        message.setSenderPhoneNumber(currentUser.getMobileNumber());
        message.setMessage(request.message().trim());

        return toChatResponse(poolChatMessageRepository.save(message));
    }

    public PoolResponse payout(UUID poolId, PayoutRequest request, String userEmail) {
        UserAccount currentUser = findUser(userEmail);
        Pool pool = findPool(poolId);
        ensurePoolMember(pool, currentUser);

        if (pool.isPayoutCompleted()) {
            throw new BadRequestException("Payout has already been completed for this pool");
        }

        long collectedAmount = collectedAmount(pool);
        if (collectedAmount <= 0) {
            throw new BadRequestException("No funds are available for payout yet");
        }

        boolean force = request != null && request.force();
        boolean normalEligible = isPayoutEligible(pool);
        boolean forceEligible = isForcePayoutEligible(pool);

        if (force) {
            ensureAdmin(pool, currentUser);
            if (!forceEligible) {
                throw new BadRequestException("Force payout is available only after 50% of the target is collected");
            }
        } else if (!normalEligible) {
            throw new BadRequestException("Payout becomes available once the full target amount is reached");
        }

        pool.setPayoutCompleted(true);
        pool.setPayoutType(force ? "FORCE" : "TARGET_REACHED");
        pool.setPayoutAmount(collectedAmount);
        pool.setPayoutTriggeredBy(currentUser.getName());
        pool.setPayoutTriggeredAt(java.time.Instant.now());

        Pool savedPool = poolRepository.save(pool);
        notifyPayout(savedPool, currentUser, collectedAmount, force);
        return toResponse(savedPool);
    }
    ...
    private PoolResponse toResponse(Pool pool) {
        ...
        return new PoolResponse(
                ...
                isPayoutEligible(pool),
                isForcePayoutEligible(pool),
                pool.isPayoutCompleted(),
                pool.getPayoutType(),
                pool.getPayoutAmount(),
                pool.getPayoutTriggeredBy(),
                pool.getPayoutTriggeredAt(),
                pool.getCreatedAt(),
                members
        );
    }
    ...
    private boolean isPayoutEligible(Pool pool) {
        return !pool.isPayoutCompleted() && collectedAmount(pool) >= pool.getTargetAmount();
    }

    private boolean isForcePayoutEligible(Pool pool) {
        return !pool.isPayoutCompleted() && collectedAmount(pool) * 2 >= pool.getTargetAmount();
    }

    private PoolChatMessageResponse toChatResponse(PoolChatMessage message) {
        return new PoolChatMessageResponse(
                message.getId(),
    ...
    private void notifyPayout(Pool pool, UserAccount actor, long collectedAmount, boolean force) {
        List<String> phoneNumbers = pool.getMembers().stream()
                .map(PoolMember::getPhoneNumber)
                .distinct()
                .toList();

        long shareAmount = pool.getMembers().isEmpty() ? collectedAmount : collectedAmount / pool.getMembers().size();
        boolean adminControl = pool.getSettlementMode() == com.gatherpay.backend.domain.PoolSettlementMode.ADMIN_CONTROL;

        userAccountRepository.findAllByMobileNumberIn(phoneNumbers).forEach(user -> {
            if (adminControl) {
                if (user.getMobileNumber().equals(actor.getMobileNumber())) {
                    notificationService.createAdminReceivedNotification(user, pool.getName(), collectedAmount);
                } else {
                    notificationService.createAdminControlledPayoutNotice(
                            user,
                            pool.getName(),
                            actor.getName(),
                            collectedAmount
            ...
            notificationService.createShareReceivedNotification(user, pool.getName(), shareAmount);
            notificationService.createPayoutReleasedNotification(user, pool.getName(), collectedAmount, force);
        });
    }
}
